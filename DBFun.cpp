...
void create_project_table(QSqlDatabase data_base){
    ...
    data_base.close();
    if(cntrl){ qDebug() << "Сreating a project_table\t\tOK"; }
    else { qDebug() << "Сreating a project_table\t\tNOT OK"; }
}
void create_task_table(QSqlDatabase data_base, int id_table){
    ...
    data_base.close();
    if(cntrl){ qDebug() << "Сreating a task_table\t\tOK"; }
    else { qDebug() << "Сreating a task_table\t\tNOT OK"; }
}
// RECORD DATA //
void record_task_data(QSqlDatabase data_base, int id_table, QVector <task_data_struct> task_data_vec){
    ...
    if (!data_base.open()){ cntrl = false; error_log += "open "; }
    QSqlQuery query(data_base);
    QString sql = "INSERT INTO tasks" + QString::number(id_table) + " (tName, tDate, tBrigade, tProgress) VALUES (:tName, :tDate, :tBrigade, :tProgress)";
    ...
    for(int i = 0; i < task_data_vec.size(); i++){
        ...
        query.bindValue(":tDate", task_data_vec[i].tDate);
...
void record_project_data(QSqlDatabase data_base, QVector <project_data_struct> project_data_vec){
    ...
    sql = "INSERT INTO projects"
          " ("
                                                               ...
                                                               "pTask, "
                                                               "pBrigade) "
                                                               "VALUES (:pName, :pImage, :pDate, :pTask, :pBrigade)";
    if(!query.prepare(sql)){ cntrl = false; error_log += "prepare ";};
    for(int i = 0; i < project_data_vec.size(); i++){
        ...
        query.bindValue(":pDate", project_data_vec[i].pDate);
...
void add_task_record(QSqlDatabase data_base, int id_table, task_data_struct task_data_vec){
    ...
    query.bindValue(":tDate", task_data_vec.tDate);
...
void add_project_record(QSqlDatabase data_base, project_data_struct project_data_vec){
    ...
    query.bindValue(":pDate", project_data_vec.pDate);
...
void read_task_table(QSqlDatabase data_base, int id_table, QVector <task_data_struct> &task_data_vec){
    ...
    if (!data_base.open()){ cntrl = false; error_log += "open ";}
    QSqlQuery query(data_base);
    QString sql = "SELECT ID, tName, tDate, tBrigade, tProgress FROM tasks" + QString::number(id_table);
    if (!query.exec(sql)){ cntrl = false; error_log += "exec ";}
    while(query.next()){
        form.tID = query.value(0).toInt();
        form.tName = query.value(1).toString();
        form.tDate = query.value(2).toString();
...
void read_project_table(QSqlDatabase data_base, QVector <project_data_struct> &project_data_vec){
    ...
    if (!data_base.open()){ cntrl = false; error_log += "open ";}
    QSqlQuery query(data_base);
    QString sql = "SELECT ID, pName, pImage, pDate, pTask, pBrigade FROM projects";
    if (!query.exec(sql)){ cntrl = false; error_log += "exec ";}
    while(query.next()){
        form.pID = query.value(0).toInt();
        form.pName = query.value(1).toString();
        form.pImage = query.value(2).toString();
        form.pDate = query.value(3).toString();
...
void delete_project(QSqlDatabase data_base, int id_table){
    ...
    if (!query.exec(sql)){ cntrl = false; error_log += "exec_t ";}                              //Переделать
...
void show_vec(QVector <task_data_struct> task_data_vec){
    ...
    for(int i = 0; i < task_data_vec.size(); i++){
        qDebug() << task_data_vec[i].tID << task_data_vec[i].tName << task_data_vec[i].tDate << task_data_vec[i].tBrigade << task_data_vec[i].tProgress;
    }
...
void show_vec(QVector <project_data_struct> project_data_vec){
    ...
    for(int i = 0; i < project_data_vec.size(); i++){
        qDebug() << project_data_vec[i].pID << project_data_vec[i].pName << project_data_vec[i].pImage << project_data_vec[i].pDate << project_data_vec[i].pTask << project_data_vec[i].pBrigade;
    }
